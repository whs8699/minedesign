...
export const Select:FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames('mc-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="mc-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
            ...
            timeout={300}
          >
          <ul className="mc-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="mc-selected-tags" style={{maxWidth: containerWidth.current - 32}}> 
          {
            selectedValues.map((value, index) => {
              return (
                <span className="mc-tag" key={`tag-${index}`}>
                  {value}
