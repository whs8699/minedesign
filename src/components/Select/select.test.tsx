...
describe('test Select component', () => {
  ...
  it('Select in multiple mode should works fine', () => {
    ...
    // add tags
    expect(container.querySelectorAll('.mc-tag').length).toEqual(1)
    //remove placeholder
    expect(inputEle.placeholder).toEqual('')
    // click 2nd item
    fireEvent.click(secondItem)
    expect(multipleProps.onChange).toHaveBeenLastCalledWith('id2', ['id1', 'id2'])
    expect(container.querySelectorAll('.mc-tag').length).toEqual(2)
    //reclick 2nd item
    fireEvent.click(secondItem)
    // remove acitve class
    ...
    //remove all tags
    expect(container.querySelectorAll('.mc-tag').length).toEqual(0)
